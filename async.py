...
async def main():
    ...
    async with httpx.AsyncClient() as client:
        ...
        list_urls = await download_photos(num_photos, base_url)
        for url in list_urls[:5]:
            print(f'Photo link: {url}')

        print("Start factorial")
        factorial_tasks = [asyncio.create_task(factorial(n)) for n in range(30)]
        ...
        if data:
            print("Received json data from the database:")
            print(data[:2])
        else:
            print("Failed to fetch json data from the database.")

        print("Start fetch data from url")
        fetch_result = await fetch_data(fetch_url_data)
        print(f"Response fetch data from {fetch_url_data}:\n {fetch_result}")
